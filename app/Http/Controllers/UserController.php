...
class UserController extends Controller
{
    ...
    public function postAccountPassword(Request $request){
        ...
        if($validator->fails()):
            ...
            if(Hash::check($request->input('apassword'), $u->password)):
                ...
                return back()->with('message','Se contraseña actual es erronea')->with('typealert','danger'); 
            endif;
        endif;
    }
    ...
    public function postAccountInfo(Request $request){
        ...
        $messages = [
            ...
            'phone.required' => 'Su número de telefon es requerido',
            'phone.min' => 'El número de teléfono debe tener como minimo 10 digitos',
        ...
        if($validator->fails()):
            ...
            $u->birthday = $request->input('year').'-'.$request->input('month').'-'.$request->input('day');
    ...
    public function postAccountAddressAdd(Request $request){
        ...
        $messages = [
            ...
            'state.required' => 'Seleccione un barrio',
            'add1.required' => 'Su número de telefon es requerido',
            'add2.min' => 'Ingrese su calle / carrera / avenida',
            'add3.required' => 'Ingrese el número de su casa / apartamento' 
        ];
    ...
    public function getAccountAddressDelete(UserAddress $address){
        if(Auth::id() != $address->user_id):
            ...
            if($address->default == "0"):
                if($address->delete()):
                    return back()->with('message','La dirección se elimino con exito.')->with('typealert','success'); 
                endif;
            else:
