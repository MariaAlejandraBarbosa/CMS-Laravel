...
class ConnectController extends Controller
{
    ...
    public function postLogin(Request $request){
        ...
        if($validator->fails()):
            ...
            if(Auth::attempt(['email' => $request->input('email'), 'password' => $request->input('password')], true)):
                ...
                return back()->withErrors($validator)->with('message','Correo electrónico o contraseña erronea')->with('typealert','danger');
            endif;
        endif;
    }
    ...
    public function postRegister(Request $request){
        ...
        if($validator->fails()):
            ...
            if($user->save()):
                return redirect('/login')->with('message','Su usuario se creo con exito, ahora puede iniciar sesión')->with(
                    'typealert','success');
            endif;
        endif;
    }
        ...
        if($validator->fails()):
            ...
            if($user == "1"):
                ...
                if($u->save()):
                    ...
                    return redirect('/reset?email='.$user->email)->with('message','Ingrese el código que le hemos enviado a su correo electrónico.')->with(
                        'typealert','success');
                    //return view('emails.user_password_recover', $data);
                endif;
            else:
                return back()->with('message','Este correo electronico no exite.')->with(
                    'typealert','danger');
            endif;
        endif;
    }
        ...
        if($validator->fails()):
            ...
            if($user == "1"):
                ...
                if($user->save()):
                    ...
                    return redirect('/login')->with('message','La contraseña fue restablecida con exito, ahora puede iniciar sesión')->with(
                        'typealert','success');
                endif;
            else:
                return back()->with('message','El correo electrónico o el código de recuperación son erróneos.')->with(
                    'typealert','danger');
            endif;
        endif;
    }
